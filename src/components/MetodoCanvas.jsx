...
export default function MetodoCanvas({ activeIndex = 0, onOrbClick }) {
  ...
  useEffect(() => {
    ...
    const drawFluidConnection = (positions) => {
      ...
      ctx.quadraticCurveTo(mx1 + (p1.y - p0.y) * 0.3 + curve1, my1 - (p1.x - p0.x) * 0.3, p1.x, p1.y);

      const mx2 = (p1.x + p2.x) / 2, my2 = (p1.y + p2.y) / 2;
      const curve2 = size() * 0.08 * Math.cos(flowPhase * 0.7);
      ctx.quadraticCurveTo(mx2 + (p2.y - p1.y) * 0.3 + curve2, my2 - (p2.x - p1.x) * 0.3, p2.x, p2.y);

      const mx3 = (p2.x + p0.x) / 2, my3 = (p2.y + p0.y) / 2;
      ctx.quadraticCurveTo(mx3 + (p0.y - p2.y) * 0.3, my3 - (p0.x - p2.x) * 0.3 + size() * 0.06 * Math.sin(flowPhase * 0.5), p0.x, p0.y);
      ctx.closePath();

      const active = positions[currentActive] || positions[0];
      const grad = ctx.createRadialGradient(active.x, active.y, 0, active.x, active.y, size() * 0.6);
  ...
  // Base orb classes
  const orbBase = "method-label-orb absolute rounded-full flex items-center justify-center font-mono tracking-[2px] max-md:tracking-[1px] text-white bg-glow-gold shadow-[0_0_40px_rgba(168,132,90,0.4)] transition-all duration-[0.8s] ease-out cursor-pointer z-[2] text-center";
  const orbSizes = "w-[130px] h-[130px] md:w-[100px] md:h-[100px] max-md:w-[85px] max-md:h-[85px] max-[480px]:w-[70px] max-[480px]:h-[70px] text-[10px] max-md:text-[8px] max-[480px]:text-[7px] p-2.5 max-md:p-1.5";

  const orbData = [
    { label: "ESCUCHA", top: "12%", left: "50%", index: 0 },
    { label: "RITUAL", top: "48%", left: "33%", index: 1 },
    { label: "RESULTADO", top: "48%", left: "67%", index: 2 },
  ];

  return (
    <div ref={containerRef} className="method-visual-container relative w-full aspect-square flex items-center justify-center goo-filter">
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none blur-[10px]" />
      ...
      {orbData.map((orb) => (
        <button
          ...
          style={{
            top: orb.top,
            left: orb.left,
            transform: "translateX(-50%) translate(var(--float-x, 0px), var(--float-y, 0px)) scale(1)",
          }}
